...
void main() {
  group('AgenticChatNotifier immutability', () {
    ...
    test(
      'changeSensitivity marks pending approvals stale via new instances',
      () async {
        final approval = _pendingApproval('a1');
        final message = _systemMessageWithApprovals([approval]);
  ...
  group('AgenticChatNotifier — queued message', () {
    ...
    test(
      'retryFailedMessages on stuck continuation keeps the SystemAction',
      () async {
        ...
        final messages = container
            .read(agenticChatProvider('test-session'))
            .messages;
        ...
        final userMessages = messages
            .where((m) => m.role == AgenticRole.user)
            .toList();
    ...
    test('retryFailedMessages keeps the trailing error in history', () async {
      ...
      notifier.setStateForTest(AgenticChatState(messages: [user, error]));
      ...
      expect(
        messages.any(
          (m) => m.messageId == error.messageId && m.role == AgenticRole.error,
        ),
        isTrue,
    ...
    test('sendMessage enqueues when awaiting and does not append', () async {
      ...
      expect(
        container.read(agenticChatProvider('test-session')).isAwaiting,
        isTrue,
      );
    ...
    test('sendMessage replaces an existing queued message', () async {
      ...
      expect(
        container.read(agenticChatProvider('test-session')).queuedMessage,
        'second',
      );
    });
    ...
    test('editQueued returns the text and clears the slot', () {
      ...
      expect(
        container.read(agenticChatProvider('test-session')).inputEnabled,
        isTrue,
      );
    });
  ...
  group('AgenticChatNotifier — loadHistory / refreshFromServer', () {
    test('sets sensitivityLevel from response on full load', () async {
      ...
      final notifier = container.read(
        agenticChatProvider('test-session').notifier,
      );
    ...
    test(
      'refreshFromServer fetches everything for a fresh instance (no cursor)',
      () async {
        Uri? requestedUri;
        final mockClient = MockClient((request) async {
        ...
        expect(
          ...
          isFalse,
          reason:
              'a fresh instance has no final messages, so its cursor is '
    ...
    test(
      'refreshFromServer fetches since the last known final message',
      () async {
        Uri? requestedUri;
        final mockClient = MockClient((request) async {
          requestedUri = request.url;
    ...
    test(
      'refreshFromServer coalesces concurrent calls into one re-fetch',
      () async {
        var requestCount = 0;
        ...
        final subscription = container.listen(
          agenticChatProvider('test-session'),
          (_, _) {},
        );
  ...
  group('AgenticChatNotifier — seedFirstExchange', () {
    test('merges the user message and response, marking both final when the '
      ...
      addTearDown(container.dispose);
      final notifier = container.read(
        agenticChatProvider('new-session').notifier,
      );
    ...
    test(
      'leaves the response non-final when it is not final (e.g. a pending approval)',
      () {
        final container = _makeContainer();
        addTearDown(container.dispose);
        final notifier = container.read(
  ...
  group('AgenticChatNotifier — per-session isolation', () {
    test('two different session ids produce independent instances', () {
      ...
      final notifierA = container.read(
        agenticChatProvider('session-a').notifier,
      );
      ...
      expect(
        ...
        isEmpty,
        reason:
            'family instances are separate objects by construction — '
  ...
  group('AgenticChatNotifier — disposal policy', () {
    test(
      'a session with an in-flight cycle is not disposed when unwatched',
      () {
        final container = _makeContainer();
        addTearDown(container.dispose);
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          hasLength(1),
          reason:
              'a quick flip back must reuse the still-warm instance, not '
    ...
    test(
      'an idle session is disposed after the linger window if never rewatched',
      () async {
        final container = _makeContainer();
        addTearDown(container.dispose);
        ...
        final freshWatch = container.listen(
          agenticChatProvider('s1'),
          (_, _) {},
        );
        final state = container.read(agenticChatProvider('s1'));
        expect(
          ...
          isEmpty,
          reason:
              'genuinely idle and unwatched past the linger window — '
  ...
  group('AgenticChatNotifier — auto-playback re-check on response arrival', () {
    test('turning on auto-playback while a request is in flight still plays '
        'the response that arrives afterward', () async {
      ...
      final notifier = container.read(
        agenticChatProvider('auto-play-session').notifier,
      );
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'auto-playback must be re-checked against current settings '
