...
void main() {
  ...
  Future<void> _createCompleteMarker(String id, ModelType type) async {
    ...
    final dir = Directory(p.join(tmpDir.path, 'imported_models', typeDir, id));
    await dir.create(recursive: true);
    await File(p.join(dir.path, '.complete')).writeAsString('done');
  }

  Future<void> _createVitsPiperLayout(String id) async {
    final dir = Directory(p.join(tmpDir.path, 'imported_models', 'tts', id));
    await dir.create(recursive: true);
  ...
  test('resolveAsrMetadata resolves the sherpa-onnx whisper export\'s '
    ...
    final dir = Directory(p.join(tmpDir.path, 'imported_models', 'asr', id));
    await dir.create(recursive: true);
    await File(
      p.join(dir.path, 'nb-whisper-base-encoder.onnx'),
    ).writeAsBytes([]);
    await File(
      p.join(dir.path, 'nb-whisper-base-decoder.onnx'),
    ).writeAsBytes([]);
    await File(
      p.join(dir.path, 'nb-whisper-base-tokens.txt'),
    ).writeAsString('');
    await File(p.join(dir.path, '.complete')).writeAsString('done');
  ...
  test(
    'resolveAsrMetadata falls back to imported model when catalog miss',
    () async {
  ...
  test(
    'resolveAsrMetadata returns null for imported model without .complete marker',
    () async {
  ...
  test(
    'resolveTtsModel falls back to imported model when catalog miss',
    () async {
