...
- Engine API: No `/status` endpoint exposing version
...
3. **Engine API Enhancement**: Add `/status` endpoint returning version and component info
...
- Affected code:
  ...
  - `engine/run.py` (add /status endpoint)
