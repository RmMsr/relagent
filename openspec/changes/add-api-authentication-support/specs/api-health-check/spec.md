...
Validate API configuration and authentication before chat operations by testing connectivity and detecting authentication requirements.

## ADDED Requirements

### Requirement: Health Check Execution

The system SHALL provide a health check mechanism to validate API endpoint configuration and authentication.

#### Scenario: Health check on app startup
- **GIVEN** the app has a configured API endpoint saved in settings
- **WHEN** the app starts up
- **THEN** a health check SHALL be performed against the chat completions endpoint
...
The system SHALL use health check results to prevent chat errors and guide users proactively.
