# bundled-inference-container Specification

## Purpose

Provide a batteries-included container image that ships a CPU-based `llama.cpp`
inference server alongside the engine, so Relagent can answer conversations with
no externally configured inference provider. The bundled image exists for
demonstration, showcasing, and first-try / evaluation use — it is explicitly not
a productive or permanent deployment option. The default engine image stays
provider-less and expects an external OpenAI-compatible endpoint.
