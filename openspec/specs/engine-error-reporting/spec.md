# engine-error-reporting Specification

## Purpose

Ensure internal engine errors that are surfaced to a client carry a
human-readable description, and that the app displays that description rather
than only an HTTP status code — so a misconfigured or not-yet-ready setup is
distinguishable from a real defect. The first concrete instance is
inference-provider unavailability, reported as a retryable `503` whose reason
