...
void main() {
  group('VoiceCatalogDiscovery.classify — NeMo transducer routing', () {
    ...
    test('cache-aware streaming NeMo transducer is routed to the '
      ...
      final entries = [
        _untestedEntry('sherpa-onnx-nemo-streaming-fast-conformer-transducer-en-80ms-int8'),
      ];

      discovery.classify(entries);

      expect(entries.single['architecture'], 'nemoCacheAwareStreamingTransducer');
    });
    ...
    test('"non-streaming" NeMo transducer export is NOT caught by the '
      ...
      final entries = [
        _untestedEntry('sherpa-onnx-nemo-parakeet-unified-en-0-6b-int8-non-streaming'),
      ];
